...
-- =============================================
-- FUNCIONES AUXILIARES PARA POL�TICAS RLS
-- =============================================
...
-- =============================================
-- POL�TICAS RLS PARA TABLA STORIES
-- =============================================
...
-- Los usuarios pueden crear historias para s� mismos
...
-- =============================================
-- POL�TICAS RLS PARA TABLA VOCABULARY_WORDS
-- =============================================
...
-- Los usuarios pueden crear palabras de vocabulario para s� mismos
...
-- =============================================
-- POL�TICAS RLS PARA TABLA STORY_PROGRESS
-- =============================================
...
-- Los usuarios pueden crear progreso para s� mismos
...
-- =============================================
-- POL�TICAS RLS PARA TABLA USAGE_ANALYTICS
-- =============================================

-- Los usuarios pueden crear eventos de analytics para s� mismos
CREATE POLICY "Users can create own analytics" ON usage_analytics
    FOR INSERT WITH CHECK (
        auth.uid() = user_id OR 
        user_id IS NULL -- Permitir eventos an�nimos
    );
...
-- =============================================
-- POL�TICAS RLS PARA TABLA AD_BANNERS
-- =============================================
...
-- =============================================
-- POL�TICAS RLS PARA TABLA USER_LIMITS
-- =============================================

-- Los usuarios pueden ver sus propios l�mites
CREATE POLICY "Users can view own limits" ON user_limits
    FOR SELECT USING (auth.uid() = user_id);

-- Solo administradores pueden crear l�mites
CREATE POLICY "Admins can create user limits" ON user_limits
    FOR INSERT WITH CHECK (is_admin());

-- Solo administradores pueden ver todos los l�mites
CREATE POLICY "Admins can view all limits" ON user_limits
    FOR SELECT USING (is_admin());

-- Solo administradores pueden actualizar l�mites
CREATE POLICY "Admins can update user limits" ON user_limits
    FOR UPDATE USING (is_admin());

-- Solo administradores pueden eliminar l�mites
CREATE POLICY "Admins can delete user limits" ON user_limits
    FOR DELETE USING (is_admin());

-- =============================================
-- POL�TICAS RLS PARA TABLA API_HEALTH_CHECKS
-- =============================================
...
-- =============================================
-- POL�TICAS ESPECIALES PARA CASOS DE USO ESPEC�FICOS
-- =============================================

-- Pol�tica para permitir que los usuarios vean historias compartidas (futuro feature)
...
-- Pol�tica para permitir lectura de stats agregadas por administradores
...
-- =============================================
-- POL�TICAS PARA OPERACIONES DE SISTEMA
-- =============================================

-- Permitir a los triggers del sistema actualizar contadores autom�ticamente
-- (usando SECURITY DEFINER en las funciones de trigger)

-- Pol�tica para permitir actualizaciones de l�mites por parte del sistema
CREATE POLICY "System can update limits" ON user_limits
    FOR UPDATE USING (current_setting('role') = 'service_role');

-- Pol�tica para permitir inserci�n autom�tica de l�mites por defecto
...
-- =============================================
-- �NDICES ADICIONALES PARA PERFORMANCE DE RLS
-- =============================================

-- �ndices para mejorar performance de pol�ticas RLS
